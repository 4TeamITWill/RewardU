...
function deleteMsg() { //삭제버튼 눌렀을 때 submit하는 함수
	document.message_form.action = './DeleteMessageCheckbox.message?divide=store';
...
<%
	...
	int count = mdao.getCountMessage(id,"store"); //해당 id가 받은 메시지가 몇 개인지 반환
	ArrayList list = mdao.getMessageList(id,startRow,pageSize,"store"); //해당 id가 받은 메시지 목록을 list로 반환
...
<div class="distinc_div">
	...
	<a href="./MemberMessage_StoreList.message"><span class="msglist_span msglist_active">메시지 보관함</span></a>
</div>
...
<tr class="table_header">
	<th>보낸 사람</th> <th>받은 사람</th> <th>제목</th> <th>날짜</th> 
...
<%
	for(int i=0; i<list.size(); i++){
		...
		if(toID.equals("rewardu4@gmail.com")){
			toID = "관리자";}
		else if(fromID.equals("rewardu4@gmail.com")){
			fromID = "관리자";}
%>
		<tr align="center">
			<td width="200px"><%=toID%></td>
			<td width="200px"><%=fromID%></td>
			<td width="500px">
			<a class="subject_a" href="./ContentMessageAction.message?no=<%=mdto.getNo()%>&divide=store&id=<%=id%>"><%=mdto.getSubject()%></a>
			</td>
...
<input type="button" value="메시지 보내기" class="msg_btn" onclick="location.href='./SendMessage.message?divide=store'">
...
%>
			<a href="./MemberMessage_StoreList.message?pageNum=<%=startPage-pageBlock%>">이전</a>
<%			
		}
		// [1][2][3]...[10]
		for(int i=startPage; i<=endPage; i++){
%>
			<a href="./MemberMessage_StoreList.message?pageNum=<%=i%>"><%=i%></a> 
<%
		}
		...
		if(endPage < pageCount){
%>
			<a href="./MemberMessage_StoreList.message?pageNum=<%=startPage+pageBlock%>">다음</a>
<%
		}	
	}
%>
