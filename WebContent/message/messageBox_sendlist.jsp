...
function deleteMsg() { //삭제버튼 눌렀을 때 submit하는 함수
	document.message_form.action = './DeleteMessageCheckbox.message?divide=send';
	document.message_form.submit();
}
function storeMsg() { //보관버튼 눌렀을 때 submit하는 함수
	document.message_form.action = './StoreMessageCheckbox.message?divide=send';
	document.message_form.submit();
}
...
<%
	...
	int count = mdao.getCountMessage(id,"send"); //해당 id가 받은 메시지가 몇 개인지 반환
	ArrayList list = mdao.getMessageList(id,startRow,pageSize,"send"); //해당 id가 받은 메시지 목록을 list로 반환
...
<div class="distinc_div">
	<a href="./MemberMessage_ReceiveList.message"><span class="msglist_span">받은 메시지</span></a>
	<a href="./MemberMessage_SendList.message"><span class="msglist_span msglist_active">보낸 메시지</span></a>
...
<tr class="table_header">
	<th>No.</th> <th>받은 사람</th> <th>제목</th> <th>날짜</th> 
...
<%
	for(int i=0; i<list.size(); i++){
		...
		//만약 사용자가 보낸 메시지를 받은사람이 관리자이면 보여지는 이름을 관리자로 변경
		String fromID = mdto.getFromID();
		if(fromID.equals("rewardu4@gmail.com")){
			fromID = "관리자";}
%>
		<tr align="center">
			<td width="80px"><%=mdto.getNo()%></td>
			<td width="230px"><%=fromID%></td>
			<td width="500px">
			<a class="subject_a" href="./ContentMessageAction.message?no=<%=mdto.getNo()%>&divide=send&id=<%=id%>"><%=mdto.getSubject()%></a>
			</td>
...
<input type="button" value="메시지 보내기" class="msg_btn" onclick="location.href='./SendMessage.message?divide=send'">
...
%>
			<a href="./MemberMessage_SendList.message?pageNum=<%=startPage-pageBlock%>">이전</a>
<%			
		}
		// [1][2][3]...[10]
		for(int i=startPage; i<=endPage; i++){
%>
			<a href="./MemberMessage_SendList.message?pageNum=<%=i%>"><%=i%></a> 
<%
		}
		...
		if(endPage < pageCount){
%>
			<a href="./MemberMessage_SendList.message?pageNum=<%=startPage+pageBlock%>">다음</a>
<%
		}	
	}
%>
